...
# Betas
with open(os.path.join(my_path, 'Data', 'complex-optimal.pkl'), 'rb') as handle:
...
mdc_aux_pwt_k = MDP.addConstrs((mdv_aux_pwt_k[(m,d,k,c)] == ptp_k[(k,c)] * mdv_aux_pwp[(m,d,k,c)] for m in M for d in D for k in K for c in C), name='con_auxiliary_pwp_k')

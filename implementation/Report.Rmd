...
R1R2_sa <- generate_summary_sa(path, dur, warm, repl)
...
R1R2_beta_data_mdp <- bind_rows(temp1, temp2) %>% mutate(pol = 'mdp')
...
R1R2_beta_data_myopic <- bind_rows(temp1, temp2) %>% mutate(pol = 'myopic')
R1R2_beta_data <- bind_rows(R1R2_beta_data_mdp, R1R2_beta_data_myopic)

path <- "R1R2R3"
R1R2R3 <- generate_summary(path, dur, warm, repl)
R1R2R3_sa <- generate_summary_sa(path, dur, warm, repl)
...
temp1 <- read_csv("Report_Stuff/R3_basic.csv") %>% mutate(type = 'basic')
temp2 <- read_csv("Report_Stuff/R3_prio.csv") %>% mutate(type = 'prio')
R1R2R3_beta_data_mdp <- bind_rows(temp1, temp2) %>% mutate(pol = 'mdp')
temp1 <- read_csv("Report_Stuff/R3_basic_myopic.csv") %>% mutate(type = 'basic')
temp2 <- read_csv("Report_Stuff/R3_prio_myopic.csv") %>% mutate(type = 'prio')
R1R2R3_beta_data_myopic <- bind_rows(temp1, temp2) %>% mutate(pol = 'myopic')
R1R2R3_beta_data <- bind_rows(R1R2R3_beta_data_mdp, R1R2R3_beta_data_myopic)
...
R1R2_policy <- (ggplot(R1R2_beta_data %>% filter(M == 0), aes(x=T, y=Val, color=C, shape=D, linetype=D)) +
    ...
    facet_wrap(pol ~ type, scales='free') +
    scale_x_continuous(breaks=seq(0,10), limits = c(1,10))) %>% ggplotly()

R1R2R3_policy <- (ggplot(R1R2R3_beta_data %>% filter(M == 0), aes(x=T, y=Val, color=C, shape=D, linetype=D)) +
    geom_line() + geom_point() +
    facet_wrap(pol ~ type, scales='free') +
    scale_x_continuous(breaks=seq(0,10), limits = c(1,10))) %>% ggplotly()
```

...
def generate_phase1_master_model(input_data, model):
    ...
    # Beta pe
    for dc in itertools.product(indices['d'], indices['c']):
        pe_constr[dc] = mast_model.getConstrByName(f'b_pe_{str(dc)[1:][:-1].replace(" ","_").replace(",","")}')
        gv_pe[dc] = mast_model.addVar(name=f'gv_b_pe_{dc}', obj=1)
        mast_model.chgCoeff(pe_constr[dc], gv_pe[dc], 1)
...
def generate_master_model(input_data, state_action_data):
    ...
    # Cost Function
    def cost_function(state: state, action: action):
        cost = 0
    ...
    def b_pe_constraint(state: state, action: action) -> constraint_parameter:
        ...
        for dc in itertools.product(indices['d'], indices['c']):
            ...
            sign[dc] = ">="
            name[dc] = f'b_pe_{str(dc)[1:][:-1].replace(" ","_").replace(",","")}'
...
def update_master_model(input_data, master_model, master_variables, master_constraints, new_state_action, sa_index):
    ...
    cost_val = cost_function(new_state_action[0], new_state_action[1])
    master_variables.append(master_model.addVar(name=f'sa_{sa_index}', obj=cost_val))

    # Modifies Constrain Values
    master_model.chgCoeff(master_constraints['b0']['b_0'], master_variables[-1], b_0_constraint(new_state_action[0], new_state_action[1]).lhs_param['b_0'])
    for tp in itertools.product(indices['t'], indices['p']):
        # Beta ue
        master_model.chgCoeff(master_constraints['ue'][tp], master_variables[-1], b_ue_constraint(new_state_action[0], new_state_action[1]).lhs_param[tp])
        # Beta uu
        master_model.chgCoeff(master_constraints['uu'][tp], master_variables[-1], b_uu_constraint(new_state_action[0], new_state_action[1]).lhs_param[tp])
        # Beta uv
        master_model.chgCoeff(master_constraints['uv'][tp], master_variables[-1], b_uv_constraint(new_state_action[0], new_state_action[1]).lhs_param[tp])

    # Beta pe
    for dc in itertools.product(indices['d'], indices['c']):
        master_model.chgCoeff(master_constraints['pe'][dc], master_variables[-1], b_pe_constraint(new_state_action[0], new_state_action[1]).lhs_param[dc])
    
    # Beta pw
    for mdc in itertools.product(indices['m'], indices['d'], indices['c']):
        if mdc[0] == 0: continue
        master_model.chgCoeff(master_constraints['pw'][mdc], master_variables[-1], b_pw_constraint(new_state_action[0], new_state_action[1]).lhs_param[mdc])
    
    # Beta ps
    for tmdc in itertools.product(indices['t'], indices['m'], indices['d'], indices['c']):
        if tmdc[1] == 0: continue
        master_model.chgCoeff(master_constraints['ps'][tmdc], master_variables[-1], b_ps_constraint(new_state_action[0], new_state_action[1]).lhs_param[tmdc])
